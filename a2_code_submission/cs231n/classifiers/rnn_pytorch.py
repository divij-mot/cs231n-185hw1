...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        hidden0 = affine_forward(features, W_proj, b_proj)
        wordembedding = word_embedding_forward(captions_in, W_embed)

        if self.cell_type == "rnn":
          hidden = rnn_forward(wordembedding, hidden0, Wx, Wh, b)
        else:
          hidden = lstm_forward(wordembedding, hidden0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        hidden = affine_forward(features, W_proj, b_proj)

        
        previous_word = torch.full((N,), self._start, dtype=torch.long, device=features.device)

        for t in range(max_length):
            word_vector = W_embed[previous_word]

            if self.cell_type == "rnn":
                h = rnn_step_forward(word_vector, hidden, Wx, Wh, b)


            scores = affine_forward(hidden, W_vocab, b_vocab)
