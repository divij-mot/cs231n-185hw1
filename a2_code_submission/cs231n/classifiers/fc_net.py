...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for l in range(1, self.num_layers):
            ...
            cache[l] = (fc_cache, norm_cache, relu_cache, do_cache)
        ...
        cache[self.num_layers] = (last_cache, None, None, None)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # reg loss (weights only)
        for l in range(1, self.num_layers + 1):
            W = self.params[f"W{l}"]
            loss += 0.5 * self.reg * np.sum(W * W)

        # ---- last layer backward (just affine) ----
        last_fc_cache, _, _, _ = cache[self.num_layers]
        dout, dW, db = affine_backward(dout, last_fc_cache)
        ...
        for l in reversed(range(1, self.num_layers)):
            fc_cache, norm_cache, relu_cache, do_cache = cache[l]
