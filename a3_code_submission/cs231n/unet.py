...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            ##################################################################
            down_block = nn.ModuleList([ResnetBlock(dim_in, dim_in, context_dim=context_dim), ResnetBlock(dim_in, dim_in, context_dim=context_dim), Downsample(dim_in, dim_out=dim_out)])
            ##################################################################
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            ...
            ##################################################################
            up_block = nn.ModuleList([Upsample(dim_in, dim_out=dim_out), ResnetBlock(dim_out * 2, dim_out, context_dim=context_dim), ResnetBlock(dim_out * 2, dim_out, context_dim=context_dim)])
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        #this one needed to look through docs, was confused
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        skips = []
        #also looked through docs for the second section to make usre i was doing this right. s
