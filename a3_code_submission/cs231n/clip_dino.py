...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    img_tensors = torch.stack([clip_preprocess(Image.fromarray(im)) for im in images]).to(device)
    image_features = clip_model.encode_image(img_tensors)
    image_features = image_features / image_features.norm(dim=-1, keepdim=True)

    sims = image_features @ text_features.t()
    best = sims.argmax(dim=1).tolist()
    pred_classes = [class_texts[i] for i in best]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        images = [clip_preprocess(Image.fromarray(img)) for img in images]
