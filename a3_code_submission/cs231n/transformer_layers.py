...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        k = self.key(key)
        v = self.value(value)
        #seems we need it to be in nhte/h format from instructions hence transpose
        q = q.view(N, S, self.n_head, E//self.n_head).transpose(1, 2)
        k = k.view(N, T, self.n_head, E//self.n_head).transpose(1, 2)
        v = v.view(N, T, self.n_head, E//self.n_head).transpose(1, 2)
        o = torch.matmul(q, k.transpose(-1, -2)) / math.sqrt(self.head_dim)
        #k q v o, o is output; in the rare rare chance anyone reads this its from https://transformer-circuits.pub/2021/framework/index.html even though others use y
        if attn_mask is not None:
          o = o.masked_fill(attn_mask[None, None, :, :] == 0, float("-inf"))
        out = torch.softmax(o, dim=-1)
        out = self.attn_drop(out)
        out = torch.matmul(out, v)  #ov circuit!
