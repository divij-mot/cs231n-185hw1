...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        sqrta = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        sqrtaminus = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)

        x_start = (x_t - sqrtaminus * noise) / sqrta
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        pred_noise = (x_t - sqrta * x_start) / sqrtaminus.clamp(min=1e-20)
        ####################################################################
        return pred_noise
    ...
    def q_sample(self, x_start, t, noise):
        ...
        ####################################################################
        sqrta = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        sqrtaminus = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        x_t = sqrta * x_start + sqrtaminus * noise
        ####################################################################
        ####################################################################
        return x_t
