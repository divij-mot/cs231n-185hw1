...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #literally have NO CLUE why the errors aren't updating on colab but the training runs?
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        encode = self.transformer(self.positional_encoding(self.patch_embed(x)))
        pool = torch.mean(encode, dim=1)
