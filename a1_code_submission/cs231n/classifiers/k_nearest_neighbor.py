...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        abterm = np.dot(X, self.X_train.T) 
        dists = np.sqrt(X_squared[:, None] + Y_squared[None, :] - 2*abterm) 
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            y_pred[i] = min([x for x in labelDict if labelDict[x] == max(labelDict.values())])
        return y_pred
